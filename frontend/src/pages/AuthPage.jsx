...
import { Sparkles, ArrowRight, AlertCircle, Loader2, CheckCircle2, ShieldCheck } from 'lucide-react';
import { useAuth } from '../contexts/AuthContext';
import { supabase } from '../lib/supabase';
import { api } from '../lib/api';

export default function AuthPage() {
  const { signInWithGoogle, signInWithGithub, signInAsDemo, user } = useAuth();
  ...
  return (
    <div className="min-h-screen bg-[#090d16] flex items-center justify-center p-4 relative overflow-hidden selection:bg-indigo-500/30 selection:text-indigo-200">
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[550px] h-[350px] bg-gradient-to-tr from-indigo-600/20 to-purple-600/15 blur-[120px] rounded-full pointer-events-none" />
      ...
      <div className="w-full max-w-md rounded-3xl bg-slate-900/90 border border-slate-800 shadow-2xl p-7 sm:p-9 relative z-10 space-y-6">
        ...
        >
          {loading ? (
            ...
            <>
              ...
              1-Click Demo Sign-In (CivicLens AI Pre-Loaded)
            </>
          )}
        </button>
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="text-[11px] uppercase font-semibold text-slate-500">Or continue with</span>
          <div className="flex-1 h-px bg-slate-800" />
        </div>
        ...
        <div className="space-y-2.5">
          <button
            ...
            type="button"
            className="w-full py-2.5 px-4 rounded-xl bg-slate-800/80 hover:bg-slate-800 border border-slate-700 text-slate-200 text-xs font-semibold flex items-center justify-center gap-3 transition-all"
          >
